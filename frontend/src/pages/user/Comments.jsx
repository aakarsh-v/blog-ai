...
import UserCommentTableItem from '../../components/user/UserCommentTableItem';
...
const Comments = () => {
    ...
    const fetchComments = async() => {
        try {
            const {data} = await axios.get('/api/auth/comments')
            if (data.success) {
        ...
        rounded-lg scrollbar-hide'>
            <table className='w-full text-sm text-gray-500'>
                ...
                <tbody>
                    ...
                    }).map((comment, index) => <UserCommentTableItem key={comment._id}
